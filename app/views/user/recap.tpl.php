            ...
            <tbody>
                <?php foreach(array_slice($vote, 0, 9) as $categoryVote) : ?>
                    <tr <?php if (isset($verification)) {
                        if (($participation->{'get'.str_replace(" ", "", $categoryVote['category'])}() === $verification->{'get'.str_replace(" ", "", $categoryVote['category'])}()) || ($participation->{'get'.str_replace(" ", "", $categoryVote['category'])}() === $verification->{'get'.str_replace(" ", "", $categoryVote['category']).'2'}()) || ($participation->{'get'.str_replace(" ", "", $categoryVote['category'])}()  === $verification->{'get'.str_replace(" ", "", $categoryVote['category']).'3'}()) || ($participation->{'get'.str_replace(" ", "", $categoryVote['category'])}() === $verification->{'get'.str_replace(" ", "", $categoryVote['category']).'4'}()) || ($participation->{'get'.str_replace(" ", "", $categoryVote['category'])}() === $verification->{'get'.str_replace(" ", "", $categoryVote['category']).'5'}()) ) : ?> class="table-success" <?php else: ?> class="table-danger" <?php endif; }?> >
